...
import {ITLStacksVelocity1155Events, ITLStacksVelocity1155, Drop} from "tl-stacks/utils/ITLStacksVelocity1155.sol";
...
contract TLStacksVelocity1155Test is Test, ITLStacksVelocity1155Events {
    bytes32 constant MINTER_ROLE = keccak256("APPROVED_MINT_CONTRACT");
    bytes32 constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    VyperDeployer vyperDeployer = new VyperDeployer();

    ITLStacksVelocity1155 stacks;
    ERC1155TL nft;
    ERC1155TL nftTwo;
    MockERC20 coin;
    ...
    address minter = address(0xabcdef);

    function setUp() public {
        stacks = ITLStacksVelocity1155(vyperDeployer.deployContract("TLStacksVelocity1155", abi.encode(address(this))));
    ...
    function test_paused(address caller) public {
        ...
        stacks.configure_drop(address(1), 1, 100, 1, address(0), address(1), 100, 100, 1, -1);
        vm.expectRevert("contract is paused");
        ...
        vm.expectRevert("contract is paused");
        stacks.mint(address(1), 1, 1, address(2));
    ...
    function test_mint_zero_tokens() public {
        ...
        stacks.mint(address(nft), 1, 0, address(this));
    }
    ...
    function test_mint_not_approved_mint_contract() public {
        ...
        stacks.mint(address(nft), 1, 1, address(this));
    ...
    function test_not_enough_eth_sent() public {
        ...
        stacks.mint{value: 0.1 ether}(address(nft), 1, 1, ben);
    }
    ...
    function test_not_enough_allowance_given() public {
        ...
        vm.expectRevert("insufficient funds");
        stacks.mint(address(nft), 1, 1, ben);
        vm.stopPrank();
    }
    ...
    function test_mint_allowance() public {
        ...
        stacks.mint{value: 0.005 ether}(address(nft), 1, 5, ben);
    ...
    function test_mint_past_duration() public {
        ...
        vm.startPrank(ben);
        stacks.mint(address(nft), 1, 5, ben);
        assert(nft.balanceOf(ben, 1) == 4);
        uint256 dropPhase = stacks.get_drop_phase(address(nft), 1);
    ...
    function test_mint_too_much_eth() public {
        ...
        vm.startPrank(ben);
        uint256 prevBalance = ben.balance;
        stacks.mint{value: 0.005 ether}(address(nft), 1, 1, ben);
    ...
    function test_mint_erc20_with_eth() public {
        ...
        stacks.mint{value: 0.001 ether}(address(nft), 1, 1, ben);
    ...
    function test_mint_for_someone_else() public {
        ...
        stacks.mint{value: 0.001 ether}(address(nft), 1, 1, chris);
        stacks.mint{value: 0.001 ether}(address(nft), 1, 1, david);
        stacks.mint{value: 0.001 ether}(address(nft), 1,  1, ben);
        vm.stopPrank();
        ...
        stacks.mint(address(nft), 1, 1, chris);
        stacks.mint(address(nft), 1, 1, david);
        stacks.mint(address(nft), 1, 1, ben);
        vm.stopPrank();
    ...
    function test_eth_velocity_mint() public {
        ...
        // pre-drop
        vm.expectRevert("you shall not mint");
        stacks.mint{value: 0.1 ether}(address(nft), 1, 1, address(this));
        ...
        emit Purchase(ben, ben, address(nft), 1, address(0), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, ben);
        assert(nft.balanceOf(ben, 1) == 20);
        assert(minterBalance - ben.balance == 2 ether);
        assert(receiver.balance - receiverBalance == 2 ether);
        Drop memory postBenDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postBenDrop.duration == 20 minutes);
        assert(postBenDrop.supply == 80);
        vm.expectEmit(true, true, true, true);
        emit Purchase(ben, ben, address(nft), 1, address(0), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, ben);
        assert(nft.balanceOf(ben, 1) == 25);
        assert(minterBalance - ben.balance == 2.5 ether);
        assert(receiver.balance - receiverBalance == 2.5 ether);
        postBenDrop = stacks.get_drop(address(nft), 1);
        ...
        vm.expectEmit(true, true, true, true);
        emit Purchase(chris, chris, address(nft), 1, address(0), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, chris);
        assert(nft.balanceOf(chris, 1) == 20);
        assert(minterBalance - chris.balance == 2 ether);
        assert(receiver.balance - receiverBalance == 2 ether);
        Drop memory postChrisDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postChrisDrop.duration == 20 minutes);
        assert(postChrisDrop.supply == 55);
        vm.expectEmit(true, true, true, true);
        emit Purchase(chris, chris, address(nft), 1, address(0), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, chris);
        assert(nft.balanceOf(chris, 1) == 25);
        assert(minterBalance - chris.balance == 2.5 ether);
        assert(receiver.balance - receiverBalance == 2.5 ether);
        postChrisDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postChrisDrop.duration == 25 minutes);
        assert(postChrisDrop.supply == 50);
        vm.expectRevert("already hit mint allowance");
        stacks.mint{value: 0.1 ether}(address(nft), 1, 1, chris);
        vm.stopPrank();
        ...
        receiverBalance = receiver.balance;
        vm.expectEmit(true, true, true, true);
        emit Purchase(david, david, address(nft), 1, address(0), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, david);
        assert(nft.balanceOf(david, 1) == 20);
        assert(minterBalance - david.balance == 2 ether);
        assert(receiver.balance - receiverBalance == 2 ether);
        Drop memory postDavidDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postDavidDrop.duration == 20 minutes);
        assert(postDavidDrop.supply == 30);
        vm.expectEmit(true, true, true, true);
        emit Purchase(david, david, address(nft), 1, address(0), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, david);
        assert(nft.balanceOf(david, 1) == 25);
        assert(minterBalance - david.balance == 2.5 ether);
        assert(receiver.balance - receiverBalance == 2.5 ether);
        postDavidDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postDavidDrop.duration == 25 minutes);
        assert(postDavidDrop.supply == 25);
        vm.expectRevert("already hit mint allowance");
        stacks.mint{value: 0.1 ether}(address(nft), 1, 1, david);
        vm.stopPrank();
        ...
        receiverBalance = receiver.balance;
        vm.expectEmit(true, true, true, true);
        emit Purchase(bsy, bsy, address(nft), 1, address(0), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, bsy);
        assert(nft.balanceOf(bsy, 1) == 20);
        assert(minterBalance - bsy.balance == 2 ether);
        assert(receiver.balance - receiverBalance == 2 ether);
        Drop memory postBsyDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postBsyDrop.duration == 20 minutes);
        assert(postBsyDrop.supply == 5);
        vm.expectEmit(true, true, true, true);
        emit Purchase(bsy, bsy, address(nft), 1, address(0), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, bsy);
        assert(nft.balanceOf(bsy, 1) == 25);
        assert(minterBalance - bsy.balance == 2.5 ether);
        assert(receiver.balance - receiverBalance == 2.5 ether);
        postBsyDrop = stacks.get_drop(address(nft), 1);
        assert(drop.duration - postBsyDrop.duration == 25 minutes);
        assert(postBsyDrop.supply == 0);
        vm.expectRevert("you shall not mint");
        stacks.mint{value: 0.1 ether}(address(nft), 1, 1, bsy);
        vm.stopPrank();
    ...
    function test_erc20_velocity_mint() public {
        ...
        coin.approve(address(stacks), 100 ether);
        vm.expectEmit(true, true, true, true);
        emit Purchase(ben, ben, address(nft), 1, address(coin), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, ben);
        assert(nft.balanceOf(ben, 1) == 20);
        ...
        assert(postBenDrop.supply == 80);
        vm.expectEmit(true, true, true, true);
        emit Purchase(ben, ben, address(nft), 1, address(coin), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, ben);
        assert(nft.balanceOf(ben, 1) == 25);
        ...
        vm.expectEmit(true, true, true, true);
        emit Purchase(chris, chris, address(nft), 1, address(coin), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, chris);
        assert(nft.balanceOf(chris, 1) == 20);
        ...
        assert(postChrisDrop.supply == 55);
        vm.expectEmit(true, true, true, true);
        emit Purchase(chris, chris, address(nft), 1, address(coin), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, chris);
        assert(nft.balanceOf(chris, 1) == 25);
        ...
        receiverBalance = coin.balanceOf(receiver);
        vm.expectEmit(true, true, true, true);
        emit Purchase(david, david, address(nft), 1, address(coin), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, david);
        assert(nft.balanceOf(david, 1) == 20);
        ...
        assert(postDavidDrop.supply == 30);
        vm.expectEmit(true, true, true, true);
        emit Purchase(david, david, address(nft), 1, address(coin), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, david);
        assert(nft.balanceOf(david, 1) == 25);
        ...
        receiverBalance = coin.balanceOf(receiver);
        vm.expectEmit(true, true, true, true);
        emit Purchase(bsy, bsy, address(nft), 1, address(coin), 20, 0.1 ether);
        stacks.mint{value: 2.1 ether}(address(nft), 1, 20, bsy);
        assert(nft.balanceOf(bsy, 1) == 20);
        ...
        assert(postBsyDrop.supply == 5);
        vm.expectEmit(true, true, true, true);
        emit Purchase(bsy, bsy, address(nft), 1, address(coin), 5, 0.1 ether);
        stacks.mint{value: 0.5 ether}(address(nft), 1, 5, bsy);
        assert(nft.balanceOf(bsy, 1) == 25);
    ...
    function test_eth_mint_fuzz(
        ...
        uint256 nftBalance = nft.balanceOf(minter, 1);
        vm.expectEmit(true, true, true, true);
        emit Purchase(minter, minter, address(nft), 1, address(0), allowance, cost);
        stacks.mint{value: allowance * cost}(address(nft), 1, allowance, minter);
        assert(nft.balanceOf(minter, 1) - nftBalance == allowance);
        assert(recipientBalance - minter.balance == allowance * cost);
        assert(receiver.balance - payoutBalance == allowance * cost);
        Drop memory dropPostMint = stacks.get_drop(address(nft), 1);
    ...
    function test_erc20_mint_fuzz(
        ...
        uint256 nftBalance = nft.balanceOf(minter, 1);
        vm.expectEmit(true, true, true, true);
        emit Purchase(minter, minter, address(nft), 1, address(coin), allowance, cost);
        stacks.mint(address(nft), 1, allowance, minter);
        assert(nft.balanceOf(minter, 1) - nftBalance == allowance);
    ...
    function test_two_mints_going_on_separate_contracts() public {
        ...
        stacks.mint(address(nft), 1, 1, ben);
        assert(stacks.get_num_minted(address(nft), 1, ben) == 1);
        assert(stacks.get_num_minted(address(nftTwo), 1, ben) == 0);
        ...
        stacks.mint(address(nftTwo), 1, 5, ben);

        assert(stacks.get_num_minted(address(nft), 1, ben) == 1);
        assert(stacks.get_num_minted(address(nftTwo), 1, ben) == 5);
    }
    ...
    function test_two_mints_going_on_same_contract() public {
        ...
        stacks.mint(address(nft), 1, 1, ben);
        assert(stacks.get_num_minted(address(nft), 1, ben) == 1);
        assert(stacks.get_num_minted(address(nft), 2, ben) == 0);
        ...
        stacks.mint(address(nft), 2, 5, ben);

        assert(stacks.get_num_minted(address(nft), 1, ben) == 1);
        assert(stacks.get_num_minted(address(nft), 2, ben) == 5);
    }
}
